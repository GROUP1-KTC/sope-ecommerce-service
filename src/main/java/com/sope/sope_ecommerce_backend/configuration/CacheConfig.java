...
public class CacheConfig {
    ...
    @Value("${spring.cache.redis.default-ttl:10m}")
    private Duration defaultTtl;

    @Value("${spring.cache.redis.cache-ttls.users:5m}")
    private Duration usersTtl;

    @Value("${spring.cache.redis.cache-ttls.products:30m}")
    private Duration productsTtl;

    @Value("${spring.cache.redis.cache-ttls.GOSHIP_ADDRESSES:7d}")
    private Duration goShipTtl;
    ...
    private void registerGoShipCaches(Map<String, RedisCacheConfiguration> configs,
                                      RedisCacheConfiguration defaultConfig) {
        configs.put("users", defaultConfig.entryTtl(usersTtl));
        configs.put("products", defaultConfig.entryTtl(productsTtl));
        configs.put(GoShipVariable.GOSHIP_NAMESPACE, defaultConfig.entryTtl(goShipTtl));
    }
    ...
    public CacheManager cacheManager(RedisConnectionFactory connectionFactory, GenericJackson2JsonRedisSerializer redisSerializer) {
        ...
        registerGoShipCaches(configs, defaultConfig);
    ...
    public CacheErrorHandler cacheErrorHandler() {
        return new CacheErrorHandler() {
            ...
            public void handleCacheGetError(RuntimeException exception, Cache cache, Object key) {
                ...
                if (cache.getName().equals(GoShipVariable.GOSHIP_NAMESPACE)) {
                    // Fallback: Gọi trực tiếp API hoặc lấy từ file cấu hình cục bộ
//                    return goShipApi.getLocations();
                }
            }
